...
# feature. This module lives in SportsData instead,
# via prefixed entityId/sk key patterns, matching how the handbook already
# stores ATHLETE#<id>/PROFILE#META etc.
#
...
def init_firebase():
    """Initializes the Firebase app (with fallbacks for local/deploy contexts) and returns client."""
    global _firebase_db
...
def save_report(report: dict, sport: str = "FIFA_WC_2026"):
    ...
    # Timestamp in IST (UTC+5:30), used in the sort key
...
def get_latest_report(sport: str = "FIFA_WC_2026"):
    ...
    try:
        table = _sports_data_table()
        resp = table.query(
            KeyConditionExpression=Key("entityId").eq(f"REPORT#{sport_key}"),
...
def resolve_review_draft(draft_id: str, decision: str, reviewed_by: str, final_data: dict = None):
    ...
    table.update_item(
        ...
        ExpressionAttributeValues={
            ...
            ":gsi1pk": f"REVIEW_STATUS#{decision}",
        },
    )
